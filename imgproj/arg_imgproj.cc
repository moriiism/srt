...
void ArgValImgproj::Init(int argc, char* argv[])
{
    progname_ = "imgproj";
...
void ArgValImgproj::Print(FILE* fp) const
{
    ...
    fprintf(fp, "%s: xlo_new__       : %e\n", __func__, xlo_new_);
    fprintf(fp, "%s: xup_new__       : %e\n", __func__, xup_new_);
...
void ArgValImgproj::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValImgproj::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "infile  xpos  ypos  theta  y_lo  y_up  nbinx_new  xlo_new  xup_new  outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
