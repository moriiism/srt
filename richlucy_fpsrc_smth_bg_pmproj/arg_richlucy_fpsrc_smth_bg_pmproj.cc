#include "arg_richlucy_fpsrc_smth_bg_pmproj.h"

// public

void ArgValRichlucyFpsrcSmthBgPmproj::Init(int argc, char* argv[])
{
    progname_ = "richlucy_fpsrc_smth_bg_pmproj";
    ...
    fixed_src_list_ = argv[iarg]; iarg++;
    ...
    ndc_            = atoi(argv[iarg]); iarg++;
    tol_dc_         = atof(argv[iarg]); iarg++;
    npm_            = atoi(argv[iarg]); iarg++;
    tol_pm_         = atof(argv[iarg]); iarg++;
    nnewton_        = atoi(argv[iarg]); iarg++;
    tol_newton_     = atof(argv[iarg]); iarg++;
    mu_             = atof(argv[iarg]); iarg++;
}

void ArgValRichlucyFpsrcSmthBgPmproj::Print(FILE* fp) const
{
    ...
    fprintf(fp, "%s: fixed_src_list_ : %s\n",
            __func__, fixed_src_list_.c_str());
...
void ArgValRichlucyFpsrcSmthBgPmproj::Null()
{
    progname_ = "";
    datafile_ = "";
    fixed_src_list_ = "";
...
void ArgValRichlucyFpsrcSmthBgPmproj::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValRichlucyFpsrcSmthBgPmproj::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "datafile  fixed_src_list  bgfile  skyfile  "
            "resp_file  eff_file  "
            "nskyx  nskyy  ndetx  ndety  "
            "outdir  outfile_head  "
            "nem  tol_em  ndc  tol_dc  npm  tol_pm  nnewton  tol_newton  mu\n",
            progname_.c_str());
    abort();
}
