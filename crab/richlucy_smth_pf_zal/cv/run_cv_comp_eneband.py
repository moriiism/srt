#!/usr/bin/env python3
#
# run_cv_comp_eneband.py
#
'''
Overview:
  run_cv.py
  To compare images between different energy bands
  use the same total counts of 30 - 70 keV data
Input:
...
rand_seed_fake_list = sys.argv[iarg]; iarg += 1
...
print("rand_seed_fake_list = ", rand_seed_fake_list)
...
(ene_band_lst, ratio_fake_lst) = LoadList2(ene_band_list)
ratio_fake_lst = [float(x) for x in ratio_fake_lst]
rec_prog_lst = LoadList1(rec_prog_list)
mu_lst = LoadList1(mu_list)
gamma_lst = LoadList1(gamma_list)
rand_seed_fake_lst = LoadList1(rand_seed_fake_list)
rand_seed_fake_lst = [int(i) for i in rand_seed_fake_lst]
...
resp_ver_joint_lst = []
ene_band_joint_lst = []
ratio_fake_joint_lst = []
rec_prog_joint_lst = []
rand_seed_fake_joint_lst = []
for target in target_lst:
    for telescope in telescope_lst:
        for data_ver in data_ver_lst:
            for resp_ver in resp_ver_lst:        
                for ene_band, ratio_fake in zip(ene_band_lst,
                                                ratio_fake_lst):
                    for rec_prog in rec_prog_lst:
                        for rand_seed_fake in rand_seed_fake_lst:
                            outdir = (work_dir + "/"
                                      + target + "/"
                                      + telescope + "/"
                                      + "data_" + data_ver + "/"
                            ...
                            ratio_fake_joint_lst.append(ratio_fake)
                            rec_prog_joint_lst.append(rec_prog)
                            rand_seed_fake_joint_lst.append(rand_seed_fake)
...
ymaeda_share_fake_dir = "/home/morii/work/arb/ana/run/fake/crab_hxt"
...
for index in range(len(outdir_lst)):
    ...
    ratio_fake = ratio_fake_joint_lst[index]
    ...
    for index_phase in range(len(phase_id_lst)):
        data_file = (ymaeda_share_dir + "/"
                     ...
                     + ene_band + "_"
                     + "x1178y1175_w80.fits")
        outfake_dir = (ymaeda_share_fake_dir + "/"
                        + "data" + "/"
                        + data_ver + "/"
                        + telescope)
        outfake_head = (telescope + "_"
                        + "phase" + "_"
                        + phase_id_lst[index_phase] + "_"
                        + ene_band + "_"
                        + "x1178y1175_w80")
        cmd = [srt_dir + "/" + "mkobs_nph" + "/" + "mkobs_nph_ratio",
               data_file, str(rand_seed_fake_joint_lst[index]),
               str(ratio_fake),
               outfake_dir, outfake_head]
        print(cmd)
        subprocess.call(cmd)
    ...
    for index_phase in range(len(phase_id_lst)):
        ...
        #                        + ene_band + "_"
        #                        + "x1178y1175_w80.fits")
        data_on_file_fake = (ymaeda_share_fake_dir + "/"
                             + "data" + "/"
                             + data_ver + "/"
                             + telescope + "/"
                             + telescope + "_"
                             ...
                             + ene_band + "_"
                             + "x1178y1175_w80" + "_"
                             + f"{rand_seed_fake_joint_lst[index]:04}.fits")
        #cmd = [srt_dir + "/" + "crab" + "/"
        #       + "calc_flux0" + "/"
        #       + "calc_flux0",
        #       data_on_file,
    ...
    for index_phase in range(len(phase_id_lst)):
        ...
        #                         + ene_band + "_"
        #                         + "x1178y1175_w80.fits")
        data_off_file_fake = (ymaeda_share_fake_dir + "/"
                              + "data" + "/"
                              + data_ver + "/"
                              + telescope + "/"
                              + telescope + "_"
    ...
    for index_phase in range(len(phase_id_lst)):
        ...
        #                     + ene_band + "_"
        #                     + "x1178y1175_w80.fits")
        data_file_fake = (ymaeda_share_fake_dir + "/"
                          + "data" + "/"
                          + data_ver + "/"
                          + telescope + "/"
                          + telescope + "_"
        ...
        print(f"{data_file_fake}  {phase_id_lst[index_phase]} " +
