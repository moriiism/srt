...
void SrtlibRlDet2::GetSkyNewFactorArr(
    ...
    double* const sky_new_factor_arr)
{
    ...
    dgemv_(transa, ndet, nsky, 1.0,
           ...
           y_dash_arr, 1,
           0.0, sky_new_factor_arr, 1);
    delete [] den_arr;
    delete [] y_dash_arr;
}


void SrtlibRlDet2::GetSkyNewArr(
    const double* const sky_arr,
...
{
    double* sky_new_factor_det1_arr = new double[nsky];
    SrtlibRlDet2::GetSkyNewFactorArr(
        sky_arr, data1_arr, bg1_arr,
        resp_norm_mat_det1_arr,
        ndet, nsky,
        sky_new_factor_det1_arr);
    double* sky_new_factor_det2_arr = new double[nsky];
    SrtlibRlDet2::GetSkyNewFactorArr(
        sky_arr, data2_arr, bg2_arr,
        resp_norm_mat_det2_arr,
        ndet, nsky,
        sky_new_factor_det2_arr);
    double* sky_new_factor_arr = new double[nsky];
    for(int isky = 0; isky < nsky; isky ++){
        sky_new_factor_arr[isky] =
            sky_new_factor_det1_arr[isky] + 
            sky_new_factor_det2_arr[isky];
    }
    for(int isky = 0; isky < nsky; isky ++){
        sky_new_arr[isky] =
            sky_new_factor_arr[isky] * sky_arr[isky];
    }
    delete [] sky_new_factor_det1_arr;
    delete [] sky_new_factor_det2_arr;
    delete [] sky_new_factor_arr;
}

void SrtlibRlDet2::Richlucy(
    FILE* const fp_log,
    ...
    double* const sky_new_arr)
{
    ...
    for(int iem = 0; iem < nem; iem ++){
        SrtlibRlDet2::GetSkyNewArr(
            sky_pre_arr, data1_arr, data2_arr,
            bg1_arr, bg2_arr,
            resp_norm_mat_det1_arr,
            resp_norm_mat_det2_arr,
