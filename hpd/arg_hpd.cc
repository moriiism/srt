...
void ArgValHpd::Init(int argc, char* argv[])
{
    progname_ = "hpd";
...
void ArgValHpd::Print(FILE* fp) const
{
...
void ArgValHpd::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValHpd::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "infile  xpos  ypos  outfile\n",
            progname_.c_str());
    abort();
}
