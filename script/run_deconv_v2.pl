...
my $mipllib = sprintf("%s/script/mipllib/mipllib.pl",
		      $ENV{'MITOOL'});
require($mipllib);
$mipllib::sevar{'progname'} = "run_deconv_v2.pl";
...
# main
{
	...
	for(my $imu = 0; $imu <= $#mu_arr; $imu ++){
		...
		my $Lip = 1.0e-10;
		$cmd = sprintf("/home/morii/work/github/moriiism/srt/script/run.deconv.v2.S.R  " .
			       "%s  %s  " . 
			       "%e  %e  %s  %s  %e" ,
			       $respdir, $datafile, 
			       $mu_arr[$imu], $beta_arr[$ibeta], $outfile_this2, "no", $Lip);
		printf("cmd = %s\n", $cmd);
		system($cmd);
	    }
	}
