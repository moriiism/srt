...
my $mipllib = sprintf("%s/script/mipllib/mipllib.pl",
		      $ENV{'MITOOL'});
require($mipllib);
$mipllib::sevar{'progname'} = "emgist_case2_ls.pl";
...
# main
{
	...
	for(my $imu = 0; $imu <= $#mu_arr; $imu ++){
		...
		my $logfile = sprintf("%s/emgist_case2_ls.log", $outdir_this2);
		...
		my $lconst = 1.0e-5;
		my $epsilon = 1.0e-15;
		$cmd = sprintf("stdbuf -oL -eL /home/morii/work/github/moriiism/srt/emgist_case2_ls/%s  " .
			       "%s  %s  %s  " . 
			       "%e  %e  " .
			       "%s  %s  " .
			       "%d  %e  %d  %e  %e  %d  %e  %e " .
