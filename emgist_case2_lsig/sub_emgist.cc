...
//                    MifFits::OutFitsImageD(outdir, outfile_head + "_mval",
//                                           tag, 2,
//                                           bitpix,
//                                           naxes, mval_arr);
//                    delete [] naxes;
//        }
//
        // printf("iem = %d, index_supp_vec.size() = %d\n", iem, (int) index_supp_vec.size());
                   ...
                   double* const out_arr)
{
    ...
    double xval1 = xval_arr[ index_supp_vec[0] ];
    double xval1_new = xval_new_arr[ index_supp_vec[0] ];
    ...
    for(int isupp = 0; isupp < nsupp - 1; isupp ++){
        theta_vec[isupp]     = log( xval2_vec[isupp] / xval1 );
        theta_new_vec[isupp] = log( xval2_new_vec[isupp] / xval1_new );
    }
    ...
    for(int istep = 0; istep < nstep; istep ++){
        ...
        for(int isupp = 0; isupp < nsupp - 1; isupp ++){
            sum_e_theta += exp(theta_this_vec[isupp]);
        }
        double xval1_this = 1.0 / (sum_e_theta + 1.0);
        vector<double> xval2_this_vec(nsupp - 1);
        for(int isupp = 0; isupp < nsupp - 1; isupp ++){
            xval2_this_vec[isupp] = xval1_this * exp(theta_this_vec[isupp]);
        }
        ...
        xval_this_arr[index_supp_st] = xval1_this;
        for(int isupp = 0; isupp < nsupp - 1; isupp ++){
            xval_this_arr[index_supp_vec[isupp + 1]] = xval2_this_vec[isupp];
        }
        
        double sum_xval_this_arr = 0.0;
        for(int isky = 0; isky < nsky; isky ++){
            sum_xval_this_arr += xval_this_arr[isky];
        }
        // printf("factor = %e, sum_xval_this_arr = %e\n", factor, sum_xval_this_arr);
        for(int isky = 0; isky < nsky; isky ++){
            xval_this_arr[isky] /= sum_xval_this_arr;
        }
                       ...
                       vector<int>* const index_supp_vec_ptr)
{
    ...
    for(int isky = 0; isky < nsky; isky ++){
        if(mval_arr[isky] < 1.0 - beta){
            ...
            out_arr[isky] = 1;
            index_supp_vec.push_back(isky);
        }
    }
    *index_supp_vec_ptr = index_supp_vec;
}
